...
params:
  data: happy_data(), # TODO this is a problem point. Need cases passed, but unsure how to pass a "default" so to speak
  filter: total_cases # TODO update this
  country: 
    label: "Country"
    value: Alaska
    input: select
    choices: !r append(state.name, "District of Columbia")
    
title: "`r params$country` Happiness Report"         
subtitle: "World Happiness Ranking"
---
...
```{r pull_out_variables_individually}
...
```{r compare_to_average fig.height = 8}
cases %>%
      mutate(case_rank =  rank(cases_per_100000, ties.method = "min")) %>%
