...
things_to_plot <- c("Forward PE Ratio", "EV/EBITDA Ratio")

# TODO first plot
# PE_EV_plot <- pivoted_data |>
#   filter(Company == params$symbol) |>
...
ggplot(
  ...
  geom_rect(
    aes(
      xmin = min,
        xmax = max,
                ymin = as.numeric(id) - 0.3,
                ymax = as.numeric(id) + 0.3),
            fill = "lightgray",
    color = "darkgray") +
  
  # Horizontal pointrange for min → max with a point at value
  geom_pointrangeh(
    aes(
      x = value,
      xmin = min,
